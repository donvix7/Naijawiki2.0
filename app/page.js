    ...
    const fetchTrendingWords = async () => {
      try {
        const response = await fetch(
          "http://wiki-server.giguild.com/api/user/word/list"
        );

        if (!response.ok) throw new Error("Failed to fetch words");

        const data = await response.json();
        // Expecting: { success: true, words: [...] }
        setTrendingWords(data.words?.slice(0, 6) || []); // show top 6
      } catch (err) {
      ...
      <main>
        ...
        <section className="bg-gradient-to-r from-primary to-accent py-20 text-white">
          <div className="container mx-auto px-6 text-center">
            ...
            <p className="text-xl mb-8 max-w-2xl mx-auto">
              Discover, learn, and contribute to the rich linguistic heritage of
              Nigeria
            </p>
        ...
        <section className="py-16 container mx-auto px-6">
          ...
          {loading ? (
            ...
            <p className="text-center text-gray-500">
              No trending words found this week.
            </p>
          ) : (
        ...
        <section className="py-16 bg-secondary text-white">
          <div className="container mx-auto px-6 text-center">
            ...
            <p className="text-xl mb-8 max-w-2xl mx-auto">
              Help preserve Nigerian languages by contributing words, meanings,
              and pronunciations
            </p>
